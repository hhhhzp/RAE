...
class LightningDiT(nn.Module):
    ...
    def forward(self, x, t=None, y=None, return_feature=False):
        """
    ...
    def forward_with_cfg(
        self, x, t, y, cfg_scale, cfg_interval=(-1e4, -1e4), interval_cfg: float = 0.0
    ):
        """
    ...
    def forward_with_autoguidance(
        ...
        half = x[
            : len(x) // 2
        ]  # cut the x by half, autoguidance does not need repeated input
        t = t[: len(t) // 2]
        y = y[: len(y) // 2]
