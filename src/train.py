...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Stage-2 transport model on RAE latents."
    )
    parser.add_argument(
        "--config",
        type=str,
...
def main():
    ...
    global_batch_size = training_cfg.get(
        "global_batch_size", None
    )  # optional global batch size for override
    ...
    checkpoint_interval = int(
        training_cfg.get("checkpoint_interval", 4)
    )  # ckpt interval is epoch based
    ...
    if eval_config:
        ...
        eval_model = eval_config.get(
            "eval_model", False
        )  # by default eval ema. This decides whether to **additionally** eval the non-ema model.
        ...
        assert (
            reference_npz_path
        ), "eval.reference_npz_path must be specified to enable evaluation."
    else:
        do_eval = False
    ...
    optimizer, optim_msg = build_optimizer(
        [p for p in model.parameters() if p.requires_grad], training_cfg
    )
    ...
    if do_eval:
        ...
        eval_hf_dataset = load_dataset(
            str(eval_data), split='validation', trust_remote_code=True
        )
        eval_dataset = HFDatasetWrapper(
            eval_hf_dataset,
        ...
        logger.info(
            f"Evaluation dataset loaded from {eval_data}, containing {len(eval_dataset)} images."
        )
    ...
    if steps_per_epoch <= 0:
        raise ValueError(
            "Gradient accumulation configuration results in zero optimizer steps per epoch."
        )
    ...
    if guidance_scale > 1.0 and guidance_method == "autoguidance":
        ...
        guid_model: Stage2ModelProtocol = instantiate_from_config(
            guidance_model_cfg
        ).to(device)
        guid_model.eval()
    ...
    zs = torch.randn(
        micro_batch_size, *latent_size, device=device, dtype=torch.float32
    )  # always use float for noise sampling
    n = micro_batch_size
    if use_guidance:
    ...
    if maybe_resume_ckpt_path is not None:
        logger.info(
            f"Experiment resume checkpoint found at {maybe_resume_ckpt_path}, automatically resuming..."
        )
    ...
    if rank == 0:
        ...
        logger.info(
            f"Training for {num_epochs} epochs, batch size {micro_batch_size} per GPU."
        )
        logger.info(
            f"Dataset contains {len(loader.dataset)} samples, {steps_per_epoch} steps per epoch."
        )
        logger.info(
            f"Running with world size {world_size}, starting from epoch {start_epoch} to {num_epochs}."
        )
        ...
        for step, (images_or_latents, labels) in enumerate(loader):
            ...
            if global_step % sample_every == 0:
                ...
                with torch.no_grad():
                    ...
                    with autocast(**autocast_kwargs):
                        samples = eval_sampler(
                            zs_samples, ema_model_fn, **visual_sample_model_kwargs
                        )[-1]
                    samples.float()
                    if use_guidance:
